...
export default function TopologyGraph() {
  return (
    <div className="relative isolate w-full overflow-hidden rounded-2xl border border-white/[0.09] bg-[#0b1220]/70 p-4 shadow-2xl shadow-black/20 backdrop-blur-sm sm:p-6">
      ...
      <svg viewBox="0 0 545 274" className="w-full overflow-visible" role="img" aria-label="Animated service dependency topology">
        <defs>
        ...
        {links.map(([x1,y1,x2,y2], i) => <line key={i} x1={x1} y1={y1} x2={x2} y2={y2} stroke="url(#link)" strokeWidth="1.25" />)}
        {links.slice(0, 5).map(([x1,y1,x2,y2], i) => <motion.circle key={`p${i}`} r="3" fill="#7dd3fc" filter="url(#glow)" initial={{ cx:x1, cy:y1, opacity:0 }} animate={{ cx:[x1,x2], cy:[y1,y2], opacity:[0,1,1,0] }} transition={{ duration: 2.8 + i*.35, delay:i*.55, repeat:Infinity, ease:"linear" }} />)}
        {nodes.map((node, i) => <g key={node.label}>
          <motion.circle cx={node.x} cy={node.y} r="17" fill="#0b1220" stroke="#38bdf8" strokeOpacity=".28" animate={{ strokeOpacity:[.25,.6,.25] }} transition={{ duration:3, delay:i*.25, repeat:Infinity }} />
