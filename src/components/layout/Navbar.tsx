...
export default function Navbar() {
  ...
  useEffect(() => {
    ...
    const focusableItems = () => Array.from(navigation?.querySelectorAll<HTMLElement>(focusableSelector) ?? []);
  ...
  return <header className={`fixed top-0 z-50 w-full transition-all duration-300 ${scrolled ? "border-b border-white/[.08] bg-[#050816]/75 shadow-[0_10px_30px_rgba(0,0,0,.08)] backdrop-blur-xl" : "bg-transparent"}`}>
    ...
    <Container><div className="flex h-[72px] items-center justify-between">
      ...
      <button ref={menuButtonRef} onClick={toggleMenu} aria-label="Toggle menu" aria-expanded={open} aria-controls="mobile-navigation" className="rounded p-2 text-slate-300 transition hover:bg-white/5 hover:text-white md:hidden">{open ? <X size={20} /> : <Menu size={21} />}</button>
    </div>
    <AnimatePresence>{open && <motion.nav ref={mobileNavigationRef} id="mobile-navigation" aria-label="Mobile navigation" initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} transition={{ duration: .2 }} className="overflow-hidden border-t border-white/[.08] bg-[#050816]/95 backdrop-blur-xl md:hidden"><div className="flex flex-col py-2">{links.map(link => <a onClick={closeMenu} key={link} href={url(link)} className="rounded px-2 py-3 text-sm text-slate-300 transition hover:bg-white/5 hover:text-white">{link}</a>)}<a onClick={closeMenu} href="#contact" className="mt-1 rounded px-2 py-3 text-sm text-sky-300">Let’s talk</a></div></motion.nav>}</AnimatePresence>
    </Container></header>;
}
