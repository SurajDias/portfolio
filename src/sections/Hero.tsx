      ...
      <Container>
        <div className="grid items-center gap-12 lg:grid-cols-[.95fr_1.05fr] lg:gap-14">
          <div className="relative z-10">
            ...
            <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: .36, duration: .55, ease: [0.22, 1, 0.36, 1] }} className="mt-9 flex flex-wrap gap-3"><Button href={RESUME_URL} target="_blank" rel="noopener noreferrer" ariaLabel="Open Suraj Dias resume PDF in a new tab; use the PDF viewer to download it">Download Resume <ArrowDownToLine size={16} /></Button><Button href="https://github.com/surajdias" variant="secondary">View GitHub <Code2 size={16} /></Button><a href="#contact" className="interactive-button inline-flex items-center gap-2 px-3 py-3 text-sm font-medium text-slate-400 hover:text-sky-200">Contact <ArrowUpRight size={15} /></a></motion.div>
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: .7 }} className="mt-11 flex items-center gap-2 text-xs text-slate-500"><Mail size={13} className="text-sky-400" /> hello@surajdias.dev</motion.div>
          </div>
          <ArchitecturePanel />
        </div>
      </Container>
